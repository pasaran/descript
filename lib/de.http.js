...
de.http.get = function(url, params, headers, datatype, outputtype) {
        ...
        req = http_.request(options, function(res) {
            ...
            res.on('end', function() {
                ...
                if (!datatype) {
                    ...
                    switch (contentType) {
                        //  FIXME: Нужно точно понять, какой тип чему соответствует.
                        ...
                        default:
                            //  Все, что не определилось выше, считаем бинарным контентом.
                            datatype = 'binary';
                    }
                }

                promise.resolve( new de.Result.Raw( Buffer.concat(data, length), datatype, outputtype ) );
            });
...
HttpResult.prototype.data = function(data) {
    this.buffers.push(data);
    this.length += data.length;
};

HttpResult.prototype.datatype = function() {
