...
de.file.get = function(filename, context) {
    ...
    if (!promise) {
        ...
        if (is_outside) {
            var message = 'path \'' + filename + '\' is outside of rootdir';
            context.error('[file.open ' + JSON.stringify(filename) + '] ' + message);
        ...
        fs_.readFile(filename, function(error, buffer) {
            if (error) {
                ...
                //  Как минимум 404.
                context.error('[file.open ' + JSON.stringify(filename) + '] ' + error.message);
...
//
//  Смысл второго и третьего параметра:
//
...
//
de.file.eval = function(filename, namespace, sandbox, context) {
    ...
    if (!promise) {
        ...
        de.file.get(filename, context)
            .done(function(/** @type {de.Result.File} */ result) {
                ...
                try {
                    evaled = de.eval(result, namespace, sandbox, filename);
                } catch (e) {
                    context.error('[file.eval ' + JSON.stringify(filename) + '] ' + e.message);
                    promise.reject( de.error({
                        id: 'EVAL_ERROR',
                        message: e.message
                    }) );
    ...
    if (!isWatched) {
        ...
        fs_.watchFile(filename, function (curr, prev) {
            if ( prev.mtime.getTime() !== curr.mtime.getTime() ) {
                de.events.trigger(type, filename);
            }
        });
    }
};
