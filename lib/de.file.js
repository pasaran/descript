...
//  В кэше хранятся promise'ы, в которых хранятся инстансы класса FileResult.
var _get_cache = {};
    ...
    if (!promise) {
        ...
        fs_.readFile(filename, function(error, content) {
            if (error) {
                ...
                promise.resolve( new FileResult(filename, /** @type {Buffer} */ content) );
            }
        });
    }
...
de.file.eval = function(filename, namespace, sandbox) {
    ...
    if (!promise) {
        ...
        de.file.get(filename)
            .then(function(/** @type {FileResult} */ content) {
                var result;
...
//  Все считанные файлы кэшируются в _get_cache.
//  В кэше лежат экземпляры класса FileResult.

function FileResult(filename, buffer) {
    this.filename = filename;
    this.buffer = buffer;
}

//  Автоматически (по расширению файла) определяем тип содержимого.
//
FileResult.prototype.datatype = function() {
...
//  Преобразуем объект в экземпляр de.Result.Raw.
//  Преобразование происходит уже в момент запроса.
//  Это позволяет хранить в кэше просто буфера и при каждом запросе
//  можно менять datatype результата.
//
FileResult.prototype.toResult = function(datatype, outputtype) {
