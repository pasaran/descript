...
//  В кэше хранятся promise'ы, в которых хранятся инстансы класса CacheItem.
var _get_cache = {};
    ...
    if (!promise) {
        ...
        fs_.readFile(filename, function(error, content) {
            if (error) {
                ...
                promise.resolve( new CacheItem(filename, /** @type {Buffer} */ content) );
            }
        });
    }
...
//  В кэше лежат экземпляры класса CacheItem.
