...
de.server.onrequest = function(req, res, context) {
    var path = de.server.route(req, res, context);
    var block = new de.Block.Include(path);

    //  FIXME: Добавить сюда ip, headers, честный path из запроса.
    var t1 = context.debug_start('[request ' + JSON.stringify(path) + '] started');
    block.run(context.query, context)
        ...
        .always(function() {
            context.debug_end('[request ' + JSON.stringify(path) + '] ended', t1);
        });
};
