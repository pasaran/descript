...
de.script.init = function(config) {
    ...
    if (!config) {
        //  Конфиг не задан, берем параметры из командной строки.
        options = nopt({
            port: String,
            socket: String,
            rootdir: String,
...
de.script.start = function() {
    if (cluster_.isMaster) {
        if (de.config.socket) {
            if ( fs_.existsSync(de.config.socket) ) {
                fs_.unlinkSync(de.config.socket);
            }
        }
...
de.script.onrequest = function(req, res, context) {
    ...
    var block = _pages[path];
    if (!block) {
        block = _pages[path] = new de.Block.Include(path);
    }
