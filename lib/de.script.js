...
de.Script.prototype.start = function() {
    ...
    if (cluster_.isMaster) {
        ...
        var server = this.servier = http_.createServer(function (req, res) {
            ...
            if (!block) {
                block = _cache[path] = new de.Block.Include(path, that);
            }

            //  FIXME: Как это побороть? Этому блоку прилетает дефолтный таймаут,
            //  но если блок внутри устанавливает себе больший таймаут,
            //  то все равно отрабатывает дефолтный.
            block.timeout = 1000000;
            block.run(context.request.query, context).then(function(result) {
