 ...
 *   - concats old and new value if both are arrays,
 ...
 *  @return {!Object}
 */
de.mergeObjects = function() {

    var isObjectSimple = function(source) {
        return typeof source === 'object' && !Array.isArray(source) && null !== source;
    };

    return function(dest, src) {
        for (var key in src) {
            var value = src[key];
            if (Array.isArray(value) && Array.isArray(dest[key])) {
                dest[key] = dest[key].concat(value);
            } else if (isObjectSimple(dest[key]) && isObjectSimple(value)) {
                dest[key] = de.mergeObjects(dest[key], value);
            } else {
                dest[key] = value;
            }
        }
