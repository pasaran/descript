...
var Promise = require('../noscript/lib/promise.js');
    ...
    if (guard && !guard(context)) {
        ...
        if (this.timeout) {
            ...
            timeout = setTimeout(function() {
                promise.resolve( new Result.Value({
                    error: {
                        reason: 'Timeout'
                    }
                }) );
...
Block.Include.prototype._run = function(promise, context) {
    ...
    $fs.readFile(filename, 'utf-8', function(error, content) {
        if (error) {
            promise.resolve( new Result.Value({
                error: {
                    reason: 'Cannot open file: ' + filename
                }
            }) );
        } else {
            try {
...
Block.Http.prototype._run = function(promise, context) {
    ...
    if (this.extend) {
        util.extends( options.query, this.getParams(context) );
    }

    options = {
        host: options.hostname,
        port: options.port || 80,
        path: $url.format({ query: options.query, pathname: options.pathname })
    };

    util.httpGet(options, function(error, result) {
        if (error) {
            promise.resolve( new Result.Value({
            ...
            switch (result.status) {
                ...
                case 404:
                    promise.resolve( new Result.Value({
                        error: {
                            reason: 'Requested url "' + url + '" not found'
                        }
                    }));

                    break;

                // TODO: Дописать 302, 403, 50x.
