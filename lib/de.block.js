...
de.Block.prototype.run = function(params, context) {
    ...
    if (!running) {
        ...
        var running = this._run(params, context);
    ...
    running.then(function(result) {
        ...
        if (options.template) {
            ...
            var error = that.checkFilename(filename);
            if (error) {
                return promise.resolve(error);
            }

            return de.file.eval(filename)
                .then(function(template) {
                    var r = template( result.object() );
                    var v;
                    if (r && typeof r === 'object') {
                        if ( Array.isArray(r) ) {
                            v = new de.Result.Array(r);
                        } else {
                            v = new de.Result.Object(r);
                        }
                    } else {
                        v = new de.Result.Value(r);
                    }
                    promise.resolve(v);
                })
...
de.Block.Call.prototype._run = function(params, context) {
    //  FIXME: В какой момент кидается ошибка о том, что нет модуля или метода?
    //  Кажется, не в тот.
    return this.call(params, context, this.method);
};
...
de.Block.Function.prototype._run = function(params, context) {
    ...
    var block = de.Block.compile(result);

    return block.fork(params, context);
};
...
de.Block.Include.prototype._run = function(params, context) {
    ...
    including
        .then(function(block) {
            //  FIXME: Или pipe или что угодно... И пробросить 'abort'.
...
de.Block.Value.prototype._run = function(params, context) {
    return ( new no.Promise() ).resolve(this.value);
};
