...
de.Block.prototype._init = function(descript, options) {
    ...
    this.before = compileObj(options.before);
    this.after = compileObj(options.after);

    this.result = compile(options.result);

    this.key = compile(options.key);
...
var nullResult = new de.Result.Value(null);
var nullPromise = ( new no.Promise() ).resolve(null_result);
...
de.Block.prototype.run = function(params, context) {
    ...
    if (_after) {
        promise.then(function(result) {
            for (var key in _before) {
                state[key] = _after[key](params, context);
            }
        });
    }
...
de.Block.Http.prototype._run = function(promise, params, context) {
    ...
    var worker = no.de.http(url, (this.extend) ? params : null);
