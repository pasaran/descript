...
de.Block.prototype._init = function(options) {
    ...
    //  Функция с сигнатурой (result, context, params),
    //  jpath или объект типа jresult.
    ...
    //  FIXME: #20
    //  _options.timeout = options.timeout || de.config.timeout || 5000;
};
...
de.Block.prototype.run = function(params, context) {
    ...
    if (!running) {
        ...
        var running = new no.Promise();

        this._run(running, params, context);
        ...
        if (options.timeout) {
            ...
            hTimeout = setTimeout(function() {
                ...
                running.reject( new de.Result.Error({
                    id: 'TIMEOUT',
    ...
    if (select) {
        running.then(function(result) {
            ...
            for (var key in select) {
                //  FIXME: Сигнатура?!
                state[key] = select[key](r, context);
            }
        });
    }
    ...
    if (options.result) {
        running.then(function(result) {
            var r = new de.Result.Value( options.result( result.object(), context ) );
...
de.Block.File.prototype._run = function(promise, params, context) {
    ...
    //  FIXME: Проверить filename на null -- и вернуть ошибку про недопустимый filename.

    var datatype = this._datatype();

    //  FIXME: Унести no.de.file в descript.
    //  Сделать, чтобы он сразу возвращал promise и резолвил/реджектил объектами de.Result.*.
    //  Плюс использовал расширение файла для определения datatype, если он не указан.
...
de.Block.File.prototype._datatype = function(filename) {
    var datatype = this.datatype;

    if (!datatype) {
        //  FIXME: Может быть унести это в no.de.file?
