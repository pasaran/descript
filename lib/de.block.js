...
de.Block.prototype.run = function(params, context) {
    ...
    running.then(function(result) {
        ...
        if (options.template) {
            //  FIXME: Таки нужен de.file.load.
            //  Тут нужно сперва асинхронно прочитать шаблон, а потом уже исполнять.
...
de.Block.File.prototype._run = function(params, context) {
    ...
    if (filename === null) {
        return ( new no.Promise() ).resolve( new de.Result.Error({
            id: 'FILE_INVALID_PATH',
            message: 'Invalid path "' + filename + '"'
        }) );
    }
