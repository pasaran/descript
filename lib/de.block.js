...
de.Block.prototype._init = function(options) {
    ...
    //  Может быть либо функцией с сигнатурой (params, context),
    //  либо строкой вида: '.id == 42 && !state.foo'
    _options.guard = compileExpr(options.guard);

    //  Функция с сигнатурой (params, context).
...
de.Block.prototype.run = function(params, context) {
    ...
    if ( o_guard && !o_guard(params, context) ) {
        return ( new no.Promise() ).resolve(null);
    }
    ...
    if (o_select) {
        promise.then(function(result) {
            ...
            for (var key in o_select) {
                state[key] = _select[key](r, context);
            }
        });
    }
    ...
    if (o_result) {
        ...
        promise.then(function(result) {
            var r = o_result( result.object(), context );
