...
de.Result.Raw = function(result, datatype, output_type) {
    /** @type {Buffer} */
    this.result = result.buffer;
    this.datatype = datatype || result.datatype;
...
//  У блока de.Result.Raw есть два флага: datatype и output_type.
...
//  т.е. output_type === 'json' (и по-прежнему datatype === 'text').
//
//  datatype может принимать одно из значений: 'json', 'text', 'html', 'binary',
...
de.Result.Raw.prototype.write = function(stream, output_type) {
    ...
    if (this.datatype === 'json' || output_type !== 'json') {
        //  Либо это у нас изначально json,
        //  либо это text, html, binary, который не внутри объекта/массива.
...
de.Result.File = function(filename, buffer) {
    ...
    this.data_type = data_type_by_content_type[this._content_type] || 'binary';
};
...
de.Result.Http = function(headers) {
    ...
    this.content_type = headers['content-type'].replace(/\s*;.*$/, '') || 'application/octet-stream';
...
//  FIXME: Не нужно это явно вызывать.
